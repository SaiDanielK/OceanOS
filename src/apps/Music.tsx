...
export default function Music() {
  ...
  const handlePrev = () => {
    setCurrentSong((prev) => {
      const next = (prev - 1 + songs.length) % songs.length;
      return next;
    });
  ...
  return (
    <div className="h-full flex flex-col text-white">
      ...
      <div className="flex flex-col items-center pt-6 px-6 shrink-0">

        <p className="text-xs tracking-[0.4em] uppercase text-white/50 mb-5">
          Now Playing
        </p>

        <div className="w-56 h-56 rounded-3xl bg-white/10 flex items-center justify-center overflow-hidden shadow-2xl">
          <img
        ...
        <div className="flex justify-center gap-8 mt-8">
          <button onClick={handlePrev} className="w-12 h-12 rounded-full bg-white/10">
            ◀
          </button>

          <button onClick={togglePlay} className="w-16 h-16 rounded-full bg-cyan-400 text-black text-2xl">
            {isPlaying ? "❚❚" : "▶"}
          </button>

          <button onClick={handleNext} className="w-12 h-12 rounded-full bg-white/10">
            ▶▶
          </button>
        </div>
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-white/20 scrollbar-track-transparent mt-6 px-4 space-y-3">
        ...
        {songs.map((song, index) => (
          <div
            ...
            className={`
              flex items-center gap-4 rounded-2xl p-3 cursor-pointer transition
              ${index === currentSong
                ? "bg-cyan-400/20"
